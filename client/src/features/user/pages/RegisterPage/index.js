  ...
  const handelClickRegister = async () => {
    ...
    if (confirmPassword.value != password.value) {
      ...
      try {
        ...
        if (res.status === 200) {
          enqueueSnackbar("Register successfully, login to continue exploring!", {
            variant: "success",
          });
          navigate("/login");
        }
      ...
      <div className={cx("container")}>
        <div className={cx("header")}>
          <div className={cx("login-icon")}>
            <HttpsOutlinedIcon sx={{ fontSize: "2.5rem" }} />
          </div>
          <p>Register</p>
        </div>
        <Grid container spacing={3}>
          ...
          <Grid item xs={9}>
            ...
            <p className={cx("link")} onClick={() => navigate("/login")}>
